...
TARGET_VENDOR_F27 = "10950"  # Lipari
...
# Helpers
# =============================================================================
def get_department_number(sku: str) -> int:
    try:
        cursor.execute("""
            SELECT DPT.F03
...
def sku_belongs_to_vendor(conn_cursor, sku: str, vendor_f27: str) -> bool:
    """
    Validate that SKU (F01) belongs to the expected vendor (F27=10950) in COST_TAB.
    We accept a match if there exists at least one COST_TAB row with same F01 and F27.
    """
    try:
        conn_cursor.execute(
            "SELECT TOP 1 1 FROM dbo.COST_TAB WHERE F01 = ? AND F27 = ?",
            (str(sku).strip(), str(vendor_f27).strip())
        )
        return conn_cursor.fetchone() is not None
    except Exception as e:
        log_warn(f"Vendor validation query failed for SKU={sku} vendor={vendor_f27}: {type(e).__name__}: {e}")
        # If validation itself fails, be safe and say "not valid"
        return False
...
for filename in files:
    # Keep legacy filter: must be CSV and must contain vendor id in filename
    if not filename.lower().endswith(".csv") or TARGET_VENDOR_F27 not in filename:
        skipped_files += 1
        continue
    ...
    try:
        with open(input_csv_path, newline='', encoding="utf-8-sig") as csvfile:
            ...
            for row in reader:
                ...
                # 1) LipariItem -> SKU (legacy mapping)
                ...
                # 2) NEW: validate vendor (F27 must be 10950)
                if not sku_belongs_to_vendor(cursor, sku, TARGET_VENDOR_F27):
                    skipped_vendor_mismatch += 1
                    log_warn(
                        f"Vendor mismatch: SKU={sku} is not linked to vendor F27={TARGET_VENDOR_F27} | "
                ...
                # Keep legacy behavior: quantity as-is, date as-is
                try:
    ...
    log_info(
        ...
        f"skipped_vendor_mismatch={skipped_vendor_mismatch} dept0={dept0_count}"
    )
